...
public partial class BreadcrumbView : UserControl
{
    ...
    {
        if (e.Source is not TextBox {DataContext: Observers.Breadcrumb breadcrumb} textBox) return;
        ...
        var vm = this.GetControl<ItemsControl>("ItemsControl").DataContext as ViewModels.NodeDetailViewModel;
        vm!.InFocus = false;
    }

    private void CrumbEntryKeyDown(object? sender, KeyEventArgs e)
    {
        if (e.Source is not TextBox {DataContext: Observers.Breadcrumb breadcrumb}) return;
        ...
        itemsControl.Focus();
        var vm = itemsControl.DataContext as ViewModels.NodeDetailViewModel;
        vm!.InFocus = false;
