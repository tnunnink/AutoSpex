...
namespace AutoSpex.Engine.Tests.Migrations;

[TestFixture]
public class SpecMigrationV1Tests
{
    [Test]
    public Task Run_SimpleExample_ShouldBeVerified()
    {
        const string json = """
                            {
        ...
        var migration = new SpecMigrationV1();

        var result = migration.Run(json);

        return VerifyJson(result);
    }

    [Test]
    public Task Run_ComplexExample_ShouldBeVerified()
    {
        const string json = """
                            {
